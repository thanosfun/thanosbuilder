...
function kernelsu() {
    if [ "$KERNELSU" = "yes" ];then
          ...
          if [ ! -f "${MainPath}/KernelSU/README.md" ]; then
             cd ${MainPath}
             curl -LSs "https://raw.githubusercontent.com/thanosfun/thanosbuilder/refs/heads/main/ksu_setup.sh" | bash -
...
export ARCH="arm64"
export KBUILD_BUILD_USER="ThanOS"
export KBUILD_BUILD_HOST="GoogleCloudPlatform"
export KERNEL_NAME="AvengersKernel"
